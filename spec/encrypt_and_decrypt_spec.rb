...
module SheToldMeSheHadAGodComplexIToldHerIWasAnAtheist
  describe 'encrypting and decrypting' do
    let(:passwords) { Passwords.new do |passwords|
                        passwords.add 'name1', 'password' => 'pass1', 'search_words' => 'search words1'
                        passwords.add 'name2', 'password' => 'pass2', 'search_words' => 'search words2'
                      end }
    ...
    specify 'encrypting a passwords collection does not mutate it' do
      ...
      passwords.map(&:search_words).should =~ ['search words1', 'search words2']
    end
    ...
    specify 'decrypting a passwords collection returns the same collection that was encrypted' do
      ...
      decrypted.map(&:search_words).should =~ ['search words1', 'search words2']
    end
  end
end
